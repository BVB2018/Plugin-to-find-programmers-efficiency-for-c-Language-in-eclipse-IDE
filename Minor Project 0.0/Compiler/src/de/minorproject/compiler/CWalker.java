...
public class CWalker extends CBaseListener {
				     ...
				     try{
				      ...
				      if(isValidInteger)
				    	  if((Integer.bitCount(intob)==1)&&(intob>1)){
				    		  System.out.println("mul Div Exp Not Eff on line: "+ctx.getStart().getLine());
				...
				if(flag1==0){
					...
					if(Count_IF<2){
						...
						if((ctx.getChild(5).getText().equals("else"))&&(Exp1==1)&&(Exp2==1)){
							System.out.println("can be replaced with ternary on line: "+ctx.getStart().getLine());
						...
						if(sw==1){
							...
							System.out.println("can be replaced with switch on line: "+ctx.getStart().getLine());
						}
						if(sw==-1)
							Constants.ifEff++;
					}
			...
			if(!(ob.equals("*")||(ob.equals("/"))))
				...
				boolean isValidInteger = false;
			     try{
			      ...
			      if(isValidInteger)
			    	  if((Integer.bitCount(intob)==1)&&(intob>1)){
			    		  System.out.println("mul Div Exp is ineff on line:  "+ctx.getStart().getLine());
			    		  Constants.mulDivExpNotEff++;
				     }else{
				    	 Constants.mulDivExpEff++;
				     }
	...
	public void exitIterationStatement(IterationStatementContext ctx){
		...
		if(ForCondState==1){
			...
			System.out.println("inefficient loop on line: "+ctx.getStart().getLine());
